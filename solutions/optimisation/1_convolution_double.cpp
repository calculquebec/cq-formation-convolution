...
static void prod_conv(LePNG & rgba, const Noyau & filtre)
{
    ...
    typedef struct {double r, g, b;} double_rgb;
    std::vector<double_rgb> dim_temp(im_temp.size());
    for(png_uint_32 i = 0; i < im_temp.size(); i++) {
        dim_temp[i].r = double(im_temp[i].r);
        dim_temp[i].g = double(im_temp[i].g);
        dim_temp[i].b = double(im_temp[i].b);
    }
        ...
        for (int j = 0; j < largeur; ++j) {
            ...
            for (int ii = -marge; ii <= marge; ++ii) {
                ...
                for (int jj = -marge; jj <= marge; ++jj) {
                    r += dim_temp[index_im + jj].r * filtre[index_filt - jj];
                    g += dim_temp[index_im + jj].g * filtre[index_filt - jj];
                    b += dim_temp[index_im + jj].b * filtre[index_filt - jj];
                }
            }
