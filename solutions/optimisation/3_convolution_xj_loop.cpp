...
int main(int inArgc, char *inArgv[])
{
    ...
    double lR, lG, lB;    
    double lRed[lWidth], lGreen[lWidth], lBlue[lWidth];    
        ...
        for (int j = -lHalfK; j <= lHalfK; j++) {
            ...
            for(int x = lHalfK; x < (int)lWidth - lHalfK; x++)
            {
                ...
                for (int i = -lHalfK; i <= lHalfK; i++) {
                    ...
                    lRed[x-lHalfK]   += dlImage[dlImageoff    ] * lFilter[fx + fy*lK];
                    lGreen[x-lHalfK] += dlImage[dlImageoff + 1] * lFilter[fx + fy*lK];
                    lBlue[x-lHalfK]  += dlImage[dlImageoff + 2] * lFilter[fx + fy*lK];
                    dlImageoff += 4;
                }
            }
	}
