...
unsigned char convolution_one_pixel_color(const double *lIm, const double *lFilter,
					  int lWidth, int lK)
{
    ...
    for (fy = 0; fy < lK; fy++) {
        for (fx = 0; fx < lK; fx++) {
            ...
            l += lIm[fx + fy*lWidth] * lFilter[fx + fy*lK];
        }
    }
...
void convolution_one_pixel(unsigned char *outImage, const double *lImage, const double *lFilter,
			   int lWidth, int lHalfK, int lK, int area)
{
    //Placer le résultat dans l'image.
    const double *lIm = &lImage[-lHalfK*lWidth - lHalfK];
    outImage[0] = convolution_one_pixel_color(&lIm[     0], lFilter, lWidth, lK);
    outImage[1] = convolution_one_pixel_color(&lIm[  area], lFilter, lWidth, lK);
    outImage[2] = convolution_one_pixel_color(&lIm[2*area], lFilter, lWidth, lK);
    outImage[3] = lImage[3*area];
}
...
int main(int inArgc, char *inArgv[])
{
    ...
    int area = lWidth * lHeight;
    double *dlImageT = new double[area*4];
